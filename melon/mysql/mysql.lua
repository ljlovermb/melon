...
local function _read_auth_result(self, old_auth_data, plugin)
    local packet, typ, err = _recv_packet(self)
    if not packet then
        return nil, nil, "failed to receive the r.lua packet: " .. err
    end
...
local function _read_ok_result(self)
    local packet, typ, err = _recv_packet(self)
    if not packet then
        return "failed to receive the r.lua packet: " .. err
    end
            ...
            if status == 4 then
                if self.is_unix or self.use_ssl then
                    ...
                    if not public_key then
                        ...
                        if not packet then
                            return "failed to receive the r.lua packet: "
                                .. err
                        end
...
local function read_result(self, est_nrows)
    if self.state ~= STATE_COMMAND_SENT then
        return nil, "cannot read r.lua in the current context: "
                    .. (self.state or "nil")
    end
    ...
    --print("read the r.lua set header packet")
